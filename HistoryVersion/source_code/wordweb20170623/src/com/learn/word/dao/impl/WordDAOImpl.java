...
public class WordDAOImpl extends BaseDAO<Word> implements WordDAO {
	...
	public int getTotalListWordNumber(ListWord lw) {
		// TODO Auto-generated method stub
		String sql = "SELECT count(name) FROM testword2 where " + "l_na = ?";
		Object ob = getSingelValue(sql, lw.getListName());
	...
	public List<Word> getListWordPageList(ListWord lw, int pageSize) {
		// TODO Auto-generated method stub
		String sql = "select * from (select rownum \"rowNum\", temp.* from (select  G_ID \"gId\",l_id \"lId\",NAME \"name\",PRON \"pron\",P \"p\",M \"m\",MDATE \"mDate\",L_NA \"lNa\",MEANING \"meaning\",BOOK_NAME \"bookName\",MTIME \"mTime\"  from testword2 where L_NA  = ? order by g_id) temp)where \"rowNum\" between ? and ?";
		return queryForList(sql, lw.getListName(), ((lw.getPageNo() - 1) * pageSize + 1), pageSize * lw.getPageNo());
	}
