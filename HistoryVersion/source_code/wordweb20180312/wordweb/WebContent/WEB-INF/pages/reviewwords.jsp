	...
	$(function(){
		...
		$("#pageNo").change(function(){
			...
			if(reg.test(val)){
				...
				pageNo = parseInt(val);
				if(pageNo >= 1 && pageNo <= parseInt("${reviewwordpage.totalPageNumber }")){
					flag = true;
				}
			}
			...
			//3. 页面跳转
			var href = "wordServlet?method=getReviewWords&pageNo=" + pageNo + "&" + $(":hidden").serialize();
...
<body  bgcolor="#adfedc">
	<br>
		<form target="_blank" action="wordServlet?method=getWords" method="post">
		...
		<table  style="margin:0 0 5 750">
				<tr>
					...
					<td>
					<a target="_blank" href="wordServlet?method=forwardPage&page=base&pageNo=${reviewwordpage.pageNo }&backpath=${backpath}">
	...
	<center>
 		...
 		<table border="2" cellpadding="10" cellspacing="1">
			...
			<c:forEach items="${reviewwordpage.list }" var="word">
				<tr>
					...
					<td align="right">
					<a href="wordServlet?method=addToMBase&time=re&pageNo=${reviewwordpage.pageNo }&gId=${word.gId}&name=${word.name}">
					...
					<a href="wordServlet?method=getWord&pageNo=${reviewwordpage.pageNo }&gId=${word.gId}&backpath=${backpath}">
		...
		<font size = "5" face="Arial, Helvetica">共 ${reviewwordpage.totalPageNumber } 页</font>
		&nbsp;&nbsp;
			
		<font size = "5" face="Arial, Helvetica">当前第 ${reviewwordpage.pageNo } 页</font>
		
		&nbsp;&nbsp;
		
		<c:if test="${reviewwordpage.hasPrev }">
			<a href="wordServlet?method=getReviewWords&pageNo=1">
			<font size = "5" face="Arial, Helvetica">首页</font>
			</a>
			&nbsp;&nbsp;
			<a href="wordServlet?method=getReviewWords&pageNo=${reviewwordpage.prevPage }">
		...
		<c:if test="${reviewwordpage.hasNext }">
			<a href="wordServlet?method=getReviewWords&pageNo=${reviewwordpage.nextPage }">
			...
			&nbsp;&nbsp;
			<a href="wordServlet?method=getReviewWords&pageNo=${reviewwordpage.totalPageNumber }">
