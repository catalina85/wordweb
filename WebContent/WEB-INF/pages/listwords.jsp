	...
	$(function(){
		...
		$("#pageNo").change(function(){
			...
			if(reg.test(val)){
				...
				pageNo = parseInt(val);
				if(pageNo >= 1 && pageNo <= parseInt("${listwordpage.totalPageNumber }")){
					flag = true;
				}
			}
			...
			//3. 页面跳转
			var href = "wordServlet?method=getListWords&pageNo=" + pageNo + "&" + $(":hidden").serialize();
...
<body bgcolor="#adfedc">
	<br>
		...
		<table  style="margin:0 0 5 750">
				<tr>
					...
					<td>
					<a href="wordServlet?method=forwardPage&page=base&pageNo=${listwordpage.pageNo }&backpath=${backpath}" target="_blank">
	...
	<center>
 		...
 		<table border="2" cellpadding="10" cellspacing="1">
			...
			<c:forEach items="${listwordpage.list }" var="word">
				<tr>
					...
					<td align="right">
					<a href="wordServlet?method=addToMBase&time=ln&pageNo=${listwordpage.pageNo }&gId=${word.gId}&name=${word.name}">
					...
					<a href="wordServlet?method=getWord&pageNo=${listwordpage.pageNo }&gId=${word.gId}&backpath=${backpath}">
		...
		<font size = "5" face="Arial, Helvetica">共 ${listwordpage.totalPageNumber } 页</font>
		&nbsp;&nbsp;
			
		<font size = "5" face="Arial, Helvetica">当前第 ${listwordpage.pageNo } 页</font>
		
		&nbsp;&nbsp;
		
		<c:if test="${listwordpage.hasPrev }">
			<a href="wordServlet?method=getListWords&pageNo=1">
			<font size = "5" face="Arial, Helvetica">首页</font>
			</a>
			&nbsp;&nbsp;
			<a href="wordServlet?method=getListWords&pageNo=${listwordpage.prevPage }">
		...
		<c:if test="${listwordpage.hasNext }">
			<a href="wordServlet?method=getListWords&pageNo=${listwordpage.nextPage }">
			...
			&nbsp;&nbsp;
			<a href="wordServlet?method=getListWords&pageNo=${listwordpage.totalPageNumber }">
